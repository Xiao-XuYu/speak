import{a as f,b as m,c as X,d as i,f as C,g as k,h as V,i as W,j as Y,k as Z,l as J,m as h}from"./chunk-TAES34DD.js";var U=class{constructor(){this.routes=new Map,this.guards=[],this.current=null,this._pageInstance=null}addRoute(e,a){this.routes.set(e,a)}beforeEach(e){this.guards.push(e)}async navigate(e){if(e!==this.current){if(location.hash!=="#"+e){location.hash=e;return}await this._render(e)}}async start(){window.addEventListener("hashchange",()=>this._handleHash()),await this._handleHash()}async _handleHash(){let e=location.hash.replace(/^#/,"")||"/";await this._render(e)}async _render(e){let a=this.routes.get(e)||this.routes.get("/404");if(!a){console.warn(`[router] no route for "${e}"`);return}for(let o of this.guards)if(!await o(e))return;typeof this._pageInstance?.unmount=="function"&&this._pageInstance.unmount();let t=await a();this._pageInstance=t.default||t,f.emit("route:change",{path:e,page:this._pageInstance}),this.current=e}},S=new U;var D=class extends HTMLElement{connectedCallback(){this.innerHTML=`
      <div class="flex h-screen flex-col">
        <header class="flex items-center px-3 sm:px-5 py-3 border-b border-slate-200 dark:border-slate-700 gap-3 shrink-0">
          <h1 class="text-base font-semibold">\u6717\u8BFB</h1>
    ...
    `,this.mainEl=this.querySelector("#main"),this.settingsEl=this.querySelector("#app-settings"),this.querySelector("#settings-btn").addEventListener("click",()=>{this.settingsEl?.open?.()}),f.on("route:change",({page:e})=>{this.mainEl.innerHTML="",typeof e.mount=="function"&&e.mount(this.mainEl)}),this.querySelector("#env-info").textContent=`PORT=${window.__ENV__?.PORT||"?"} \xB7 API=${window.__ENV__?.API_BASE||"(none)"}`}};customElements.define("app-shell",D);var L={play:'<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>',pause:'<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M6 5h4v14H6zM14 5h4v14h-4z"/></svg>',prev:'<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h2v12H6zM9.5 12l8.5 6V6z"/></svg>',next:'<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M16 6h2v12h-2zM6 18l8.5-6L6 6z"/></svg>'},N=class extends HTMLElement{connectedCallback(){this.innerHTML=`
    ...
    `,this._prevBtn=this.querySelector("#pb-prev"),this._playBtn=this.querySelector("#pb-play"),this._nextBtn=this.querySelector("#pb-next"),this._playIconEl=this.querySelector("#pb-play-icon"),this._statusEl=this.querySelector("#pb-status"),this._state={total:0,index:-1,playing:!1,paused:!1},this._prevBtn.addEventListener("click",()=>f.emit("player:cmd",{action:"prev"})),this._nextBtn.addEventListener("click",()=>f.emit("player:cmd",{action:"next"})),this._playBtn.addEventListener("click",()=>{let e=this._state.paused?"resume":this._state.playing?"pause":"play";f.emit("player:cmd",{action:e})}),f.on("playback:state",e=>this._render(e))}_render(e){this._state={total:Number(e?.total)||0,index:Number.isInteger(e?.index)?e.index:-1,playing:!!e?.playing,paused:!!e?.paused};let{total:a,index:t,playing:o,paused:x}=this._state,d=a>0;this._prevBtn.disabled=!d,this._nextBtn.disabled=!d,this._playBtn.disabled=!d,this._playIconEl.innerHTML=o?L.pause:L.play,d?x&&t>=0?this._statusEl.textContent=`\u5DF2\u6682\u505C \xB7 \u7B2C ${t+1} / ${a} \u6BB5`:o&&t>=0?this._statusEl.textContent=`\u6B63\u5728\u6717\u8BFB \xB7 \u7B2C ${t+1} / ${a} \u6BB5`:this._statusEl.textContent=`\u5171 ${a} \u6BB5 \xB7 \u70B9 \u25B6 \u5F00\u59CB\u6717\u8BFB`:this._statusEl.textContent="\u7C98\u8D34\u6587\u672C\u540E\u70B9 \u2728 AI \u62C6\u5206\u5F00\u59CB\u6717\u8BFB"}};customElements.define("app-player-bar",N);var H=class extends HTMLElement{connectedCallback(){this._built||(this._build(),this._built=!0)}_build(){let e=i.get(),a=i.isConfigured(),t=k("zh"),o=k("en");this.innerHTML=`
      <dialog id="dlg" class="p-0 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 backdrop:bg-black/40 max-w-[720px] w-[min(720px,calc(100vw-32px))] max-h-[calc(100vh-32px)] border border-slate-200 dark:border-slate-700">
        <form method="dialog" class="flex flex-col max-h-[calc(100vh-32px)]">
          ...
          <div class="overflow-y-auto p-5 flex flex-col gap-3 sm:gap-4">
            ...
            ${this._renderTtsSection("en","English TTS",o)}
    ...
    `}_wireAi(){let e=i.get();this.querySelector("#ai-key").value=e.apiKey,this.querySelector("#ai-base").value=e.baseUrl,this.querySelector("#ai-model").value=e.model,this.querySelector("#ai-fast-model").value=e.fastModel,this.querySelector("#ai-use-fast-model").checked=e.fastModelEnabled===!0,this.querySelector("#ai-thinking").checked=e.thinkingEnabled!==!1;let a=this.querySelector("#ai-status"),t=this.querySelector("#ai-msg"),o=this.querySelector("#ai-reply"),x=c=>{a.textContent=c?"\u2713 \u5DF2\u914D\u7F6E":"\u672A\u914D\u7F6E",a.className="ml-auto text-xs px-2 py-0.5 rounded-full "+(c?"bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-200":"bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-200")},d=(c,p="info")=>{t.textContent=c||"",t.className="text-xs "+(p==="ok"?"text-green-600 dark:text-green-400":p==="err"?"text-red-600 dark:text-red-400":"text-slate-500")},T=()=>({apiKey:this.querySelector("#ai-key").value.trim(),baseUrl:this.querySelector("#ai-base").value.trim()||i.DEFAULTS.baseUrl,model:this.querySelector("#ai-model").value.trim()||i.DEFAULTS.model,fastModel:this.querySelector("#ai-fast-model").value.trim()||i.DEFAULTS.fastModel,fastModelEnabled:this.querySelector("#ai-use-fast-model").checked===!0,thinkingEnabled:this.querySelector("#ai-thinking").checked===!0});this.querySelector("#ai-save").addEventListener("click",()=>{let c=T();if(!c.apiKey){d("\u8BF7\u586B\u5199 API Key","err");return}i.save(c),x(!0),d(`\u5DF2\u4FDD\u5B58 (${new Date().toLocaleTimeString()})`,"ok"),o.classList.add("hidden"),o.textContent="",this._refreshMinimaxVoices("zh"),this._refreshMinimaxVoices("en")}),this.querySelector("#ai-reset").addEventListener("click",()=>{i.clear(),this.querySelector("#ai-key").value="",this.querySelector("#ai-base").value="",this.querySelector("#ai-model").value="",this.querySelector("#ai-fast-model").value="",this.querySelector("#ai-use-fast-model").checked=!1,this.querySelector("#ai-thinking").checked=!0,x(!1),d("\u5DF2\u6E05\u7A7A\uFF08\u4E0B\u6B21\u4FDD\u5B58\u4F1A\u7528 .env \u9ED8\u8BA4\u503C\uFF09"),o.classList.add("hidden"),o.textContent=""});let v=this.querySelector("#ai-test");v.addEventListener("click",async()=>{let c=T();if(!c.apiKey){d("\u8BF7\u586B\u5199 API Key","err");return}i.save(c),v.disabled=!0;let p=v.textContent;v.textContent="\u6D4B\u8BD5\u4E2D...",d("\u6B63\u5728\u53D1\u9001\u8BF7\u6C42...","info"),o.classList.add("hidden"),o.textContent="";try{let y=await i.test(c,{timeoutMs:3e4});d(`\u8054\u901A\u6210\u529F (${new Date().toLocaleTimeString()})`,"ok"),o.textContent=y,o.classList.remove("hidden"),x(!0),this._refreshMinimaxVoices("zh"),this._refreshMinimaxVoices("en")}catch(y){d(`\u8054\u901A\u5931\u8D25\uFF1A${y?.message||String(y)}`,"err"),x(!1)}finally{v.disabled=!1,v.textContent=p}})}_renderTtsSection(e,a,t){let o=t.provider==="edge"?"edge-tts-api":t.provider==="minimax"?"MiniMax T2A":"\u6D4F\u89C8\u5668\u81EA\u5E26",x=t.provider==="minimax"||t.provider==="edge"?"bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-200":"bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300";return`
      <section data-tts-lang="${e}" class="border-t border-slate-200 dark:border-slate-700 pt-4 flex flex-col gap-3">
        <div class="flex items-center gap-2">
          <strong>${n(a)}</strong>
          <span data-role="status" class="ml-auto text-xs px-2 py-0.5 rounded-full ${x}">
            \u5F53\u524D: ${n(o)}
          </span>
        </div>
        <p class="text-xs text-slate-500 -mt-1">
    ...
    `}_$(e,a){return this.querySelector(`section[data-tts-lang="${e}"] [data-role="${a}"]`)}_wireTts(e){let a=k(e);if(!this.querySelector(`section[data-tts-lang="${e}"]`))return;let o=this._$(e,"provider"),x=this._$(e,"browser-block"),d=this._$(e,"edge-block"),T=this._$(e,"minimax-block"),v=this._$(e,"status"),c=this._$(e,"msg");o.value=a.provider;let p=(s,r)=>{let l=this._$(e,s),u=this._$(e,r),g=()=>{u.textContent=l.value};l.addEventListener("input",g),g()};p("browser-rate","browser-rate-val"),p("browser-pitch","browser-pitch-val"),p("browser-volume","browser-volume-val"),p("edge-rate","edge-rate-val"),p("edge-pitch","edge-pitch-val"),p("edge-volume","edge-volume-val"),p("minimax-speed","minimax-speed-val"),p("minimax-vol","minimax-vol-val"),p("minimax-pitch","minimax-pitch-val"),this._$(e,"edge-base").value=a.edgeBaseUrl,this._$(e,"minimax-base").value=a.minimaxBaseUrl,this._$(e,"minimax-model").value=a.minimaxModel;let y=this._$(e,"browser-voice"),P=()=>{let s=W();y.innerHTML='<option value="">(\u7CFB\u7EDF\u9ED8\u8BA4)</option>'+s.map(l=>`<option value="${B(l.name)}">${n(l.name)} (${n(l.lang||"?")})</option>`).join("");let r=k(e).voice;r&&(y.value=r)};P();let Q=Y(P);this._ttsCleanups.push(Q);let M=this._$(e,"edge-voice"),I=[],R=async()=>{M.innerHTML='<option value="">\u8F7D\u5165\u4E2D\u2026</option>';try{let s=await Z(this._$(e,"edge-base").value.trim()||h.DEFAULT_EDGE_BASE_URL);I=s.voices,M.innerHTML=I.map(l=>`<option value="${B(l.shortName)}">${n(l.shortName)} \u2014 ${n(l.friendlyName||l.gender||"")}</option>`).join("");let r=k(e).voice;r&&(M.value=r),b(`\u5DF2\u52A0\u8F7D ${s.count} \u4E2A edge \u8BED\u97F3`,"ok")}catch(s){M.innerHTML='<option value="">(\u52A0\u8F7D\u5931\u8D25\uFF0C\u70B9\u53F3\u4FA7\u5237\u65B0\u91CD\u8BD5)</option>',b("\u52A0\u8F7D edge \u8BED\u97F3\u5931\u8D25\uFF1A"+(s?.message||s),"err")}},$=this._$(e,"minimax-voice"),F=this._$(e,"minimax-voice-meta"),A=async()=>{$.innerHTML='<option value="">\u8F7D\u5165\u4E2D\u2026</option>';let s=i.get().apiKey,r=this._$(e,"minimax-base").value.trim()||h.DEFAULT_MINIMAX_BASE_URL;try{let l=await J(r,s),u=l.voices&&l.voices.length?l.voices:C,g=!!(l.voices&&l.voices.length);$.innerHTML=u.map(_=>{let j=_.voice_id||_.shortName,G=_.voice_name||_.friendlyName||"";return`<option value="${B(j)}">${n(j)}${G?" \u2014 "+n(G):""}${_.source&&_.source!=="system"?" ("+_.source+")":""}</option>`}).join("");let K=k(e).minimaxVoice;K&&($.value=K),F.textContent=g?`\u5171 ${u.length} \u4E2A`:`\u79BB\u7EBF\u515C\u5E95 (${u.length} \u4E2A\u5E38\u7528\u97F3\u8272;\u586B\u597D API Key \u540E\u70B9\u300C\u5237\u65B0\u97F3\u8272\u5217\u8868\u300D)`,b(g?`\u5DF2\u52A0\u8F7D ${u.length} \u4E2A MiniMax \u97F3\u8272`:"\u672A\u914D\u7F6E API Key,\u4F7F\u7528\u79BB\u7EBF\u515C\u5E95\u6E05\u5355","ok")}catch(l){$.innerHTML=C.map(g=>`<option value="${B(g.voice_id)}">${n(g.voice_id)} \u2014 ${n(g.voice_name)}</option>`).join("");let u=k(e).minimaxVoice;u&&($.value=u),F.textContent=`\u52A0\u8F7D\u5931\u8D25,\u4F7F\u7528\u79BB\u7EBF\u515C\u5E95 (${C.length} \u4E2A)`,b("\u52A0\u8F7D MiniMax \u97F3\u8272\u5931\u8D25:"+(l?.message||l),"err")}};this[`_refreshMinimaxVoices_${e}`]=A;let q=s=>{let r=s==="edge"?"edge-tts-api":s==="minimax"?"MiniMax T2A":"\u6D4F\u89C8\u5668\u81EA\u5E26",l=s==="minimax"||s==="edge"?"bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-200":"bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300";v.textContent=`\u5F53\u524D: ${r}`,v.className="ml-auto text-xs px-2 py-0.5 rounded-full "+l},b=(s,r="info")=>{c.textContent=s||"",c.className="text-xs "+(r==="ok"?"text-green-600 dark:text-green-400":r==="err"?"text-red-600 dark:text-red-400":"text-slate-500")};o.addEventListener("change",()=>{let s=o.value;x.classList.toggle("hidden",s!=="browser"),d.classList.toggle("hidden",s!=="edge"),T.classList.toggle("hidden",s!=="minimax"),q(s),s==="edge"&&I.length===0&&R(),s==="minimax"&&A()}),a.provider==="edge"&&R(),a.provider==="minimax"&&A();let O=()=>{let s=o.value,r={provider:s,edgeBaseUrl:this._$(e,"edge-base").value.trim()||h.DEFAULT_EDGE_BASE_URL,minimaxBaseUrl:this._$(e,"minimax-base").value.trim()||h.DEFAULT_MINIMAX_BASE_URL,minimaxModel:this._$(e,"minimax-model").value||h.DEFAULT_MINIMAX_MODEL};return s==="browser"?{...r,voice:y.value.trim(),rate:Number(this._$(e,"browser-rate").value),pitch:Number(this._$(e,"browser-pitch").value),volume:Number(this._$(e,"browser-volume").value)}:s==="edge"?{...r,voice:M.value.trim(),rate:Number(this._$(e,"edge-rate").value),pitch:Number(this._$(e,"edge-pitch").value),volume:Number(this._$(e,"edge-volume").value)}:{...r,minimaxVoice:$.value.trim(),minimaxSpeed:Number(this._$(e,"minimax-speed").value),minimaxVol:Number(this._$(e,"minimax-vol").value),minimaxPitch:Number(this._$(e,"minimax-pitch").value)}};this._$(e,"save").addEventListener("click",()=>{let s=O();V(e,s),b(`\u5DF2\u4FDD\u5B58 (${new Date().toLocaleTimeString()})`,"ok"),q(s.provider)});let z=this._$(e,"minimax-refresh");z&&z.addEventListener("click",()=>A());let E=this._$(e,"test");E.addEventListener("click",async()=>{let s=O();V(e,s),q(s.provider),E.disabled=!0;let r=E.textContent;E.textContent="\u64AD\u653E\u4E2D...",b("\u6B63\u5728\u5408\u6210\u5E76\u64AD\u653E...","info");try{let l=e==="en"?"Hello, this is a test read. You should hear the configured voice and speed.":"\u4F60\u597D,\u8FD9\u662F\u6D4B\u8BD5\u6717\u8BFB\u3002\u53EF\u4EE5\u542C\u5230\u5F53\u524D\u8BED\u97F3\u4E0E\u8BED\u901F\u3002";await h.speak({text:l,lang:e,...s},{onStart:()=>b("\u64AD\u653E\u4E2D...","info"),onEnd:()=>b(`\u64AD\u653E\u5B8C\u6BD5 (${new Date().toLocaleTimeString()})`,"ok"),onError:u=>b("\u64AD\u653E\u5931\u8D25\uFF1A"+(u?.message||u),"err")})}catch(l){b("\u64AD\u653E\u5931\u8D25\uFF1A"+(l?.message||l),"err")}finally{E.disabled=!1,E.textContent=r}}),this._$(e,"stop").addEventListener("click",()=>{h.stop(),b("\u5DF2\u505C\u6B62","info")})}_refreshMinimaxVoices(e){this[`_refreshMinimaxVoices_${e}`]?.()}_renderEnvSection(){return`
      <section class="border-t border-slate-200 dark:border-slate-700 pt-4 flex flex-col gap-3">
        <strong>\u73AF\u5883\u53D8\u91CF (\u6765\u81EA .env)</strong>
        <table class="w-full text-sm">
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">PORT</th><td class="py-1 font-mono">${m("PORT","?")}</td></tr>
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">API_BASE</th><td class="py-1 font-mono">${m("API_BASE","(\u672A\u8BBE\u7F6E)")}</td></tr>
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">AI_BASE_URL</th><td class="py-1 font-mono">${m("AI_BASE_URL","(\u672A\u8BBE\u7F6E)")}</td></tr>
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">AI_MODEL</th><td class="py-1 font-mono">${m("AI_MODEL","(\u672A\u8BBE\u7F6E)")}</td></tr>
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">AI_FAST_MODEL</th><td class="py-1 font-mono">${m("AI_FAST_MODEL","(\u672A\u8BBE\u7F6E)")}</td></tr>
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">DEBUG</th><td class="py-1 font-mono">${m("DEBUG","false")}</td></tr>
        </table>
    ...
    `}_wireLocalData(){this.querySelector("#clear-storage").addEventListener("click",()=>{confirm("\u786E\u8BA4\u6E05\u7A7A\u6240\u6709\u672C\u5730\u6570\u636E?")&&X.clear()})}open(){this._built||(this._build(),this._built=!0),this._dlg.showModal()}close(){this._dlg.close()}disconnectedCallback(){for(let e of this._ttsCleanups||[])e?.()}};function n(w){return String(w??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function B(w){return String(w??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}customElements.define("app-settings",H);S.addRoute("/",()=>import("./speak.js"));S.addRoute("/speak",()=>import("./speak.js"));S.addRoute("/404",()=>import("./speak.js"));console.info("[app] starting, port=",m("PORT","?"),"API_BASE=",m("API_BASE","(none)"));S.start();window.__app={router:S};
