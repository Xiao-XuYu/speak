import{a as I,b as F,c as te,d as y,f as j,g as A,h as z,i as se,j as ae,k as le,l as re,m as T,n as ie,o as P,p as R,q as B,r as oe,s as ne,t as de,u as ce}from"./chunk-LUY3X4E5.js";var O=class{constructor(){this.routes=new Map,this.guards=[],this.current=null,this._pageInstance=null}addRoute(e,s){this.routes.set(e,s)}beforeEach(e){this.guards.push(e)}async navigate(e){if(e!==this.current){if(location.hash!=="#"+e){location.hash=e;return}await this._render(e)}}async start(){window.addEventListener("hashchange",()=>this._handleHash()),await this._handleHash()}async _handleHash(){let e=location.hash.replace(/^#/,"")||"/";await this._render(e)}async _render(e){let s=this.routes.get(e)||this.routes.get("/404");if(!s){console.warn(`[router] no route for "${e}"`);return}for(let a of this.guards)if(!await a(e))return;typeof this._pageInstance?.unmount=="function"&&this._pageInstance.unmount();let t=await s();this._pageInstance=t.default||t,I.emit("route:change",{path:e,page:this._pageInstance}),this.current=e}},N=new O;var K=class extends HTMLElement{connectedCallback(){this.innerHTML=`
    ...
    `,this.mainEl=this.querySelector("#main"),this.settingsEl=this.querySelector("#app-settings"),this.querySelector("#settings-btn").addEventListener("click",()=>{this.settingsEl?.open?.()}),I.on("route:change",({path:e,page:s})=>{this.mainEl.innerHTML="",typeof s.mount=="function"&&s.mount(this.mainEl)}),this.querySelector("#env-info").textContent=`PORT=${window.__ENV__?.PORT||"?"} \xB7 API=${window.__ENV__?.API_BASE||"(none)"}`}};customElements.define("app-shell",K);var V={play:'<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>',pause:'<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M6 5h4v14H6zM14 5h4v14h-4z"/></svg>',prev:'<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h2v12H6zM9.5 12l8.5 6V6z"/></svg>',next:'<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M16 6h2v12h-2zM6 18l8.5-6L6 6z"/></svg>'},X=class extends HTMLElement{connectedCallback(){this.innerHTML=`
      <div class="fixed bottom-0 inset-x-0 z-30 pointer-events-none">
        <div class="pointer-events-auto
          ...
          <button id="pb-play" type="button" aria-label="\u64AD\u653E/\u6682\u505C"
            ...
            <span id="pb-play-icon" class="inline-flex items-center justify-center">${V.play}</span>
          </button>
    ...
    `,this._prevBtn=this.querySelector("#pb-prev"),this._playBtn=this.querySelector("#pb-play"),this._nextBtn=this.querySelector("#pb-next"),this._playIconEl=this.querySelector("#pb-play-icon"),this._statusEl=this.querySelector("#pb-status"),this._state={total:0,index:-1,playing:!1,paused:!1},this._prevBtn.addEventListener("click",()=>I.emit("player:cmd",{action:"prev"})),this._nextBtn.addEventListener("click",()=>I.emit("player:cmd",{action:"next"})),this._playBtn.addEventListener("click",()=>{let e=this._state.paused?"resume":this._state.playing?"pause":"play";I.emit("player:cmd",{action:e})}),I.on("playback:state",e=>this._render(e))}_render(e){this._state={total:Number(e?.total)||0,index:Number.isInteger(e?.index)?e.index:-1,playing:!!e?.playing,paused:!!e?.paused};let{total:s,index:t,playing:a,paused:n}=this._state,i=s>0;this._prevBtn.disabled=!i,this._nextBtn.disabled=!i,this._playBtn.disabled=!i,this._playIconEl.innerHTML=a?V.pause:V.play,i?n&&t>=0?this._statusEl.textContent=`\u5DF2\u6682\u505C \xB7 \u7B2C ${t+1} / ${s} \u6BB5`:a&&t>=0?this._statusEl.textContent=`\u6B63\u5728\u6717\u8BFB \xB7 \u7B2C ${t+1} / ${s} \u6BB5`:this._statusEl.textContent=`\u5171 ${s} \u6BB5 \xB7 \u70B9 \u25B6 \u5F00\u59CB\u6717\u8BFB`:this._statusEl.textContent="\u7C98\u8D34\u6587\u672C\u540E\u70B9 \u2728 AI \u62C6\u5206\u5F00\u59CB\u6717\u8BFB"}};customElements.define("app-player-bar",X);var G=class extends HTMLElement{connectedCallback(){this._built||(this._build(),this._built=!0)}_build(){let e=y.get(),s=y.isConfigured(),t=A("zh"),a=A("en");this.innerHTML=`
      ...
      <section class="border-t border-slate-200 dark:border-slate-700 pt-4 flex flex-col gap-3">
        ...
        <label class="flex flex-col gap-1">
          ...
          <input id="ai-base" type="text" autocomplete="off"
                 class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md focus:border-blue-500 focus:outline-none bg-white dark:bg-slate-900 font-mono"
                 placeholder="${k(y.DEFAULTS.baseUrl)}" />
        </label>
        ...
        <label class="flex flex-col gap-1">
          ...
          <input id="ai-model" type="text" autocomplete="off"
                 class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md focus:border-blue-500 focus:outline-none bg-white dark:bg-slate-900 font-mono"
                 placeholder="${k(y.DEFAULTS.model)}" />
        </label>
        ...
        <label class="flex flex-col gap-1">
          ...
          <input id="ai-fast-model" type="text" autocomplete="off"
                 class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md focus:border-blue-500 focus:outline-none bg-white dark:bg-slate-900 font-mono"
                 placeholder="${k(y.DEFAULTS.fastModel)}" />
        </label>
    ...
    `}_wireAi(){let e=y.get();this.querySelector("#ai-key").value=e.apiKey,this.querySelector("#ai-base").value=e.baseUrl,this.querySelector("#ai-model").value=e.model,this.querySelector("#ai-fast-model").value=e.fastModel,this.querySelector("#ai-use-fast-model").checked=e.fastModelEnabled===!0,this.querySelector("#ai-thinking").checked=e.thinkingEnabled!==!1;let s=this.querySelector("#ai-status"),t=this.querySelector("#ai-msg"),a=this.querySelector("#ai-reply"),n=l=>{s.textContent=l?"\u2713 \u5DF2\u914D\u7F6E":"\u672A\u914D\u7F6E",s.className="ml-auto text-xs px-2 py-0.5 rounded-full "+(l?"bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-200":"bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-200")},i=(l,r="info")=>{t.textContent=l||"",t.className="text-xs "+(r==="ok"?"text-green-600 dark:text-green-400":r==="err"?"text-red-600 dark:text-red-400":"text-slate-500")},x=()=>({apiKey:this.querySelector("#ai-key").value.trim(),baseUrl:this.querySelector("#ai-base").value.trim()||y.DEFAULTS.baseUrl,model:this.querySelector("#ai-model").value.trim()||y.DEFAULTS.model,fastModel:this.querySelector("#ai-fast-model").value.trim()||y.DEFAULTS.fastModel,fastModelEnabled:this.querySelector("#ai-use-fast-model").checked===!0,thinkingEnabled:this.querySelector("#ai-thinking").checked===!0});this.querySelector("#ai-save").addEventListener("click",()=>{let l=x();if(!l.apiKey){i("\u8BF7\u586B\u5199 API Key","err");return}y.save(l),n(!0),i(`\u5DF2\u4FDD\u5B58 (${new Date().toLocaleTimeString()})`,"ok"),a.classList.add("hidden"),a.textContent="",this._refreshMinimaxVoices("zh"),this._refreshMinimaxVoices("en")}),this.querySelector("#ai-reset").addEventListener("click",()=>{y.clear(),this.querySelector("#ai-key").value="",this.querySelector("#ai-base").value="",this.querySelector("#ai-model").value="",this.querySelector("#ai-fast-model").value="",this.querySelector("#ai-use-fast-model").checked=!1,this.querySelector("#ai-thinking").checked=!0,n(!1),i("\u5DF2\u6E05\u7A7A\uFF08\u4E0B\u6B21\u4FDD\u5B58\u4F1A\u7528 .env \u9ED8\u8BA4\u503C\uFF09"),a.classList.add("hidden"),a.textContent=""});let d=this.querySelector("#ai-test");d.addEventListener("click",async()=>{let l=x();if(!l.apiKey){i("\u8BF7\u586B\u5199 API Key","err");return}y.save(l),d.disabled=!0;let r=d.textContent;d.textContent="\u6D4B\u8BD5\u4E2D...",i("\u6B63\u5728\u53D1\u9001\u8BF7\u6C42...","info"),a.classList.add("hidden"),a.textContent="";try{let o=await y.test(l,{timeoutMs:3e4});i(`\u8054\u901A\u6210\u529F (${new Date().toLocaleTimeString()})`,"ok"),a.textContent=o,a.classList.remove("hidden"),n(!0),this._refreshMinimaxVoices("zh"),this._refreshMinimaxVoices("en")}catch(o){i(`\u8054\u901A\u5931\u8D25\uFF1A${o?.message||String(o)}`,"err"),n(!1)}finally{d.disabled=!1,d.textContent=r}})}_renderTtsSection(e,s,t){let a=t.provider==="edge"?"edge-tts-api":t.provider==="minimax"?"MiniMax T2A":"\u6D4F\u89C8\u5668\u81EA\u5E26",n=t.provider==="minimax"||t.provider==="edge"?"bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-200":"bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300";return`
        ...
        <div data-role="edge-block" class="flex flex-col gap-3 ${t.provider==="edge"?"":"hidden"}">
          <label class="flex flex-col gap-1">
            ...
            <input data-role="edge-base" type="text" autocomplete="off"
                   class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md focus:border-blue-500 focus:outline-none bg-white dark:bg-slate-900 font-mono"
                   placeholder="${k(T.DEFAULT_EDGE_BASE_URL)}" />
          </label>
          <label class="flex flex-col gap-1">
            <span class="text-sm text-slate-600 dark:text-slate-300">\u8BED\u97F3 (Voice)</span>
        ...
        <div data-role="minimax-block" class="flex flex-col gap-3 ${t.provider==="minimax"?"":"hidden"}">
          <label class="flex flex-col gap-1">
            ...
            <input data-role="minimax-base" type="text" autocomplete="off"
                   class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md focus:border-blue-500 focus:outline-none bg-white dark:bg-slate-900 font-mono"
                   placeholder="${k(T.DEFAULT_MINIMAX_BASE_URL)}" />
          </label>
          <label class="flex flex-col gap-1">
            <span class="text-sm text-slate-600 dark:text-slate-300">Model</span>
    ...
    `}_$(e,s){return this.querySelector(`section[data-tts-lang="${e}"] [data-role="${s}"]`)}_wireTts(e){let s=A(e);if(!this.querySelector(`section[data-tts-lang="${e}"]`))return;let a=this._$(e,"provider"),n=this._$(e,"browser-block"),i=this._$(e,"edge-block"),x=this._$(e,"minimax-block"),d=this._$(e,"status"),l=this._$(e,"msg");a.value=s.provider;let r=(c,v)=>{let u=this._$(e,c),$=this._$(e,v),C=()=>{$.textContent=u.value};u.addEventListener("input",C),C()};r("browser-rate","browser-rate-val"),r("browser-pitch","browser-pitch-val"),r("browser-volume","browser-volume-val"),r("edge-rate","edge-rate-val"),r("edge-pitch","edge-pitch-val"),r("edge-volume","edge-volume-val"),r("minimax-speed","minimax-speed-val"),r("minimax-vol","minimax-vol-val"),r("minimax-pitch","minimax-pitch-val"),this._$(e,"edge-base").value=s.edgeBaseUrl,this._$(e,"minimax-base").value=s.minimaxBaseUrl,this._$(e,"minimax-model").value=s.minimaxModel;let o=this._$(e,"browser-voice"),f=()=>{let c=se();o.innerHTML='<option value="">(\u7CFB\u7EDF\u9ED8\u8BA4)</option>'+c.map(u=>`<option value="${U(u.name)}">${k(u.name)} (${k(u.lang||"?")})</option>`).join("");let v=A(e).voice;v&&(o.value=v)};f();let m=ae(f);this._ttsCleanups.push(m);let g=this._$(e,"edge-voice"),M=[],E=async()=>{g.innerHTML='<option value="">\u8F7D\u5165\u4E2D\u2026</option>';try{let c=await le(this._$(e,"edge-base").value.trim()||T.DEFAULT_EDGE_BASE_URL);M=c.voices,g.innerHTML=M.map(u=>`<option value="${U(u.shortName)}">${k(u.shortName)} \u2014 ${k(u.friendlyName||u.gender||"")}</option>`).join("");let v=A(e).voice;v&&(g.value=v),h(`\u5DF2\u52A0\u8F7D ${c.count} \u4E2A edge \u8BED\u97F3`,"ok")}catch(c){g.innerHTML='<option value="">(\u52A0\u8F7D\u5931\u8D25\uFF0C\u70B9\u53F3\u4FA7\u5237\u65B0\u91CD\u8BD5)</option>',h("\u52A0\u8F7D edge \u8BED\u97F3\u5931\u8D25\uFF1A"+(c?.message||c),"err")}},q=this._$(e,"minimax-voice"),p=this._$(e,"minimax-voice-meta"),w=async()=>{q.innerHTML='<option value="">\u8F7D\u5165\u4E2D\u2026</option>';let c=y.get().apiKey,v=this._$(e,"minimax-base").value.trim()||T.DEFAULT_MINIMAX_BASE_URL;try{let u=await re(v,c),$=u.voices&&u.voices.length?u.voices:j,C=!!(u.voices&&u.voices.length);q.innerHTML=$.map(D=>{let Q=D.voice_id||D.shortName,ee=D.voice_name||D.friendlyName||"";return`<option value="${U(Q)}">${k(Q)}${ee?" \u2014 "+k(ee):""}${D.source&&D.source!=="system"?" ("+D.source+")":""}</option>`}).join("");let J=A(e).minimaxVoice;J&&(q.value=J),p.textContent=C?`\u5171 ${$.length} \u4E2A`:`\u79BB\u7EBF\u515C\u5E95 (${$.length} \u4E2A\u5E38\u7528\u97F3\u8272;\u586B\u597D API Key \u540E\u70B9\u300C\u5237\u65B0\u97F3\u8272\u5217\u8868\u300D)`,h(C?`\u5DF2\u52A0\u8F7D ${$.length} \u4E2A MiniMax \u97F3\u8272`:"\u672A\u914D\u7F6E API Key,\u4F7F\u7528\u79BB\u7EBF\u515C\u5E95\u6E05\u5355","ok")}catch(u){q.innerHTML=j.map(C=>`<option value="${U(C.voice_id)}">${k(C.voice_id)} \u2014 ${k(C.voice_name)}</option>`).join("");let $=A(e).minimaxVoice;$&&(q.value=$),p.textContent=`\u52A0\u8F7D\u5931\u8D25,\u4F7F\u7528\u79BB\u7EBF\u515C\u5E95 (${j.length} \u4E2A)`,h("\u52A0\u8F7D MiniMax \u97F3\u8272\u5931\u8D25:"+(u?.message||u),"err")}};this[`_refreshMinimaxVoices_${e}`]=w;let S=c=>{let v=c==="edge"?"edge-tts-api":c==="minimax"?"MiniMax T2A":"\u6D4F\u89C8\u5668\u81EA\u5E26",u=c==="minimax"||c==="edge"?"bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-200":"bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300";d.textContent=`\u5F53\u524D: ${v}`,d.className="ml-auto text-xs px-2 py-0.5 rounded-full "+u},h=(c,v="info")=>{l.textContent=c||"",l.className="text-xs "+(v==="ok"?"text-green-600 dark:text-green-400":v==="err"?"text-red-600 dark:text-red-400":"text-slate-500")};a.addEventListener("change",()=>{let c=a.value;n.classList.toggle("hidden",c!=="browser"),i.classList.toggle("hidden",c!=="edge"),x.classList.toggle("hidden",c!=="minimax"),S(c),c==="edge"&&M.length===0&&E(),c==="minimax"&&w()}),s.provider==="edge"&&E(),s.provider==="minimax"&&w();let Z=()=>{let c=a.value,v={provider:c,edgeBaseUrl:this._$(e,"edge-base").value.trim()||T.DEFAULT_EDGE_BASE_URL,minimaxBaseUrl:this._$(e,"minimax-base").value.trim()||T.DEFAULT_MINIMAX_BASE_URL,minimaxModel:this._$(e,"minimax-model").value||T.DEFAULT_MINIMAX_MODEL};return c==="browser"?{...v,voice:o.value.trim(),rate:Number(this._$(e,"browser-rate").value),pitch:Number(this._$(e,"browser-pitch").value),volume:Number(this._$(e,"browser-volume").value)}:c==="edge"?{...v,voice:g.value.trim(),rate:Number(this._$(e,"edge-rate").value),pitch:Number(this._$(e,"edge-pitch").value),volume:Number(this._$(e,"edge-volume").value)}:{...v,minimaxVoice:q.value.trim(),minimaxSpeed:Number(this._$(e,"minimax-speed").value),minimaxVol:Number(this._$(e,"minimax-vol").value),minimaxPitch:Number(this._$(e,"minimax-pitch").value)}};this._$(e,"save").addEventListener("click",()=>{let c=Z();z(e,c),h(`\u5DF2\u4FDD\u5B58 (${new Date().toLocaleTimeString()})`,"ok"),S(c.provider)});let W=this._$(e,"minimax-refresh");W&&W.addEventListener("click",()=>w());let H=this._$(e,"test");H.addEventListener("click",async()=>{let c=Z();z(e,c),S(c.provider),H.disabled=!0;let v=H.textContent;H.textContent="\u64AD\u653E\u4E2D...",h("\u6B63\u5728\u5408\u6210\u5E76\u64AD\u653E...","info");try{let u=e==="en"?"Hello, this is a test read. You should hear the configured voice and speed.":"\u4F60\u597D,\u8FD9\u662F\u6D4B\u8BD5\u6717\u8BFB\u3002\u53EF\u4EE5\u542C\u5230\u5F53\u524D\u8BED\u97F3\u4E0E\u8BED\u901F\u3002";await T.speak({text:u,lang:e,...c},{onStart:()=>h("\u64AD\u653E\u4E2D...","info"),onEnd:()=>h(`\u64AD\u653E\u5B8C\u6BD5 (${new Date().toLocaleTimeString()})`,"ok"),onError:$=>h("\u64AD\u653E\u5931\u8D25\uFF1A"+($?.message||$),"err")})}catch(u){h("\u64AD\u653E\u5931\u8D25\uFF1A"+(u?.message||u),"err")}finally{H.disabled=!1,H.textContent=v}}),this._$(e,"stop").addEventListener("click",()=>{T.stop(),h("\u5DF2\u505C\u6B62","info")})}_refreshMinimaxVoices(e){this[`_refreshMinimaxVoices_${e}`]?.()}_renderEnvSection(){return`
      <section class="border-t border-slate-200 dark:border-slate-700 pt-4 flex flex-col gap-3">
        <strong>\u73AF\u5883\u53D8\u91CF (\u6765\u81EA .env)</strong>
        <table class="w-full text-sm">
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">PORT</th><td class="py-1 font-mono">${F("PORT","?")}</td></tr>
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">API_BASE</th><td class="py-1 font-mono">${F("API_BASE","(\u672A\u8BBE\u7F6E)")}</td></tr>
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">AI_BASE_URL</th><td class="py-1 font-mono">${F("AI_BASE_URL","(\u672A\u8BBE\u7F6E)")}</td></tr>
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">AI_MODEL</th><td class="py-1 font-mono">${F("AI_MODEL","(\u672A\u8BBE\u7F6E)")}</td></tr>
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">AI_FAST_MODEL</th><td class="py-1 font-mono">${F("AI_FAST_MODEL","(\u672A\u8BBE\u7F6E)")}</td></tr>
          <tr><th class="text-left py-1 pr-3 text-slate-500 font-normal w-32">DEBUG</th><td class="py-1 font-mono">${F("DEBUG","false")}</td></tr>
        </table>
    ...
    `,this._body=this.querySelector("#sidebar-body"),this._collapsedBar=this.querySelector("#collapsed-bar"),this._expandedPanel=this.querySelector("#expanded-panel"),this._wireToolbar(),this._wireDialogs(),this._wireCtxMenu(),this._unsubRemote=ie(e=>{e.store!=="folders"&&e.store!=="files"||(e.store==="files"&&e.op==="del"&&e.key===this._selectedFileId&&(this._selectedFileId=null),this.refresh())}),this._render(),this.refresh())}disconnectedCallback(){this._unsubRemote?.(),document.removeEventListener("click",this._onDocClickHideMenu),document.removeEventListener("keydown",this._onEscHideMenu)}_render(){this._body&&(this._collapsed?(this._body.classList.remove("w-72","sm:w-80"),this._body.classList.add("w-10"),this._collapsedBar.classList.remove("hidden"),this._collapsedBar.classList.add("flex"),this._expandedPanel.classList.add("hidden"),this._expandedPanel.classList.remove("flex")):(this._body.classList.add("w-72","sm:w-80"),this._body.classList.remove("w-10"),this._collapsedBar.classList.add("hidden"),this._collapsedBar.classList.remove("flex"),this._expandedPanel.classList.remove("hidden"),this._expandedPanel.classList.add("flex")),this._renderTree())}_renderTree(){if(!this._expandedPanel)return;let e=this.querySelector("#tree-scroller");if(!e)return;let s=this._sortRows(this._allFolders,l=>l.name,"title"),t=this._sortRows(this._allFiles,l=>l.title,"title"),a=(this._search||"").toLowerCase(),n=a?new Set(this._allFiles.filter(l=>(l.title||"").toLowerCase().includes(a)||(l.text||"").toLowerCase().includes(a)).map(l=>l.id)):null;if(a){let l=t.filter(r=>n.has(r.id));e.innerHTML=`
      ...
      `,e.querySelectorAll("[data-search-file-id]").forEach(r=>{r.addEventListener("click",()=>{let o=r.dataset.searchFileId;this._selectedFileId=o;let f=this._allFiles.find(m=>m.id===o);if(f){this._currentFolderId=f.folderId??null;let m=f.folderId,g=new Set;for(;m!=null&&!g.has(m);)g.add(m),this._expanded.add(m),m=this._allFolders.find(M=>M.id===m)?.parentId??null}this._fireSelect(),this._render()})});return}let i=s.filter(l=>l.parentId==null),x=(l,r)=>{let o=this._expanded.has(l.id),f=s.filter(g=>g.parentId===l.id),m=t.filter(g=>g.folderId===l.id);return`
        <div>
          <div class="group flex items-center gap-1 px-1 py-1 rounded cursor-pointer text-sm
                      hover:bg-white dark:hover:bg-slate-800"
               data-folder-id="${_(l.id)}" data-depth="${r}">
            <button type="button" data-toggle="${_(l.id)}" class="shrink-0 w-5 h-5 flex items-center justify-center text-slate-400">
              ${f.length+m.length>0?o?b.chevronDown:b.chevronRight:'<span class="w-4"></span>'}
            </button>
            <span class="shrink-0 text-slate-500 dark:text-slate-400">${o?b.folderOpen:b.folder}</span>
            <span class="flex-1 truncate" title="${_(l.name)}">${_(l.name)}</span>
            <span class="text-xs text-slate-400 dark:text-slate-500">${f.length+m.length}</span>
            <button type="button" data-folder-menu="${_(l.id)}" class="opacity-0 group-hover:opacity-100 px-1 py-0.5 rounded text-slate-400 hover:text-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 dark:hover:text-slate-200 transition" aria-label="\u66F4\u591A">\u22EF</button>
          </div>
          ${o?`<div class="ml-4">
            ${f.map(g=>x(g,r+1)).join("")}
            ${m.map(g=>this._renderFileRow(g,r+1)).join("")}
          </div>`:""}
        </div>
      ...
      <div class="ml-4">
        ...
        ${i.length+d.length===0?'<div class="text-xs text-slate-400 dark:text-slate-500 px-3 py-3 text-center">\u8FD8\u6CA1\u6709\u5185\u5BB9</div>':""}
      </div>
    `,e.querySelectorAll("[data-toggle]").forEach(l=>{l.addEventListener("click",r=>{r.stopPropagation();let o=l.dataset.toggle;this._expanded.has(o)?this._expanded.delete(o):this._expanded.add(o),this._renderTree()})}),e.querySelectorAll("[data-folder-id]").forEach(l=>{l.addEventListener("click",r=>{if(r.target.closest("[data-toggle]")||r.target.closest("[data-folder-menu]"))return;let o=l.dataset.folderId;this._expanded.has(o)?this._expanded.delete(o):this._expanded.add(o),this._renderTree()})}),e.querySelectorAll("[data-folder-menu]").forEach(l=>{l.addEventListener("click",r=>{r.stopPropagation(),this._showFolderMenu(r.clientX,r.clientY,l.dataset.folderMenu)})}),e.querySelectorAll("[data-folder-id]").forEach(l=>{l.addEventListener("contextmenu",r=>{r.target.closest("[data-toggle]")||r.target.closest("[data-folder-menu]")||(r.preventDefault(),this._showFolderMenu(r.clientX,r.clientY,l.dataset.folderId))})}),e.querySelectorAll("[data-file-id]").forEach(l=>{l.addEventListener("click",()=>{this._selectedFileId=l.dataset.fileId,this._fireSelect(),this._renderTree()})}),e.querySelectorAll("[data-file-menu]").forEach(l=>{l.addEventListener("click",r=>{r.stopPropagation();let o=l.dataset.fileMenu;this._showCtxMenu(r.clientX,r.clientY,[{label:"\u91CD\u547D\u540D",icon:b.edit,onClick:()=>this._openRename("file",o)},{label:"\u590D\u5236\u4E00\u4EFD",icon:b.copy,onClick:()=>this._duplicateFile(o)},{label:"\u79FB\u52A8\u5230...",icon:b.move,onClick:()=>this._openMove("file",o)},{label:"\u5220\u9664",icon:b.trash,danger:!0,onClick:()=>this._deleteFile(o)}])})})}_renderFileRow(e,s){return`
      <div class="group flex items-center gap-1 px-1 py-1 rounded cursor-pointer text-sm
           ...
           data-file-id="${_(e.id)}" data-depth="${s}">
        <span class="shrink-0 w-5 h-5"></span>
        <span class="shrink-0 text-slate-500 dark:text-slate-400">${b.file}</span>
        <span class="flex-1 truncate" title="${_(e.title)}">${_(e.title)}</span>
        <button type="button" data-file-menu="${_(e.id)}" class="opacity-0 group-hover:opacity-100 px-1 py-0.5 rounded text-slate-400 hover:text-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 dark:hover:text-slate-200 transition" aria-label="\u66F4\u591A">\u22EF</button>
      </div>
    `}_renderSearchResultRow(e){let s=(e.text||"").replace(/\s+/g," ").slice(0,80);return`
      <button type="button" data-search-file-id="${_(e.id)}"
              class="w-full text-left px-2 py-2 rounded hover:bg-white dark:hover:bg-slate-800 transition flex flex-col gap-0.5">
        <div class="flex items-center gap-1.5 text-sm">
          <span class="shrink-0 text-slate-500 dark:text-slate-400">${b.file}</span>
          <span class="flex-1 truncate font-medium">${_(e.title)}</span>
        </div>
        ${s?`<div class="text-xs text-slate-500 dark:text-slate-400 truncate ml-6">${_(s)}\u2026</div>`:""}
      </button>
    `}_sortRows(e,s){let t=this._sortBy,a=this._sortDir==="asc"?1:-1,n=[...e];return t==="title"?n.sort((i,x)=>String(s(i)||"").localeCompare(String(s(x)||""),"zh-Hans-CN")*a):n.sort((i,x)=>((i[t]||0)-(x[t]||0))*a),n}_fireSelect(){let e=this._allFiles.find(s=>s.id===this._selectedFileId)||null;try{this._onFileSelect?.(e)}catch(s){console.error("[file-sidebar] onFileSelect threw:",s)}}_wireToolbar(){this.querySelector("#new-file").addEventListener("click",()=>this._openNewFile()),this.querySelector("#new-folder").addEventListener("click",()=>this._openNewFolder()),this.querySelector("#search-input").addEventListener("input",e=>{this._search=e.target.value.trim(),this._renderTree()}),this.querySelector("#sort-select").addEventListener("change",e=>{let[s,t]=e.target.value.split(":");this._sortBy=s,this._sortDir=t,this._renderTree()}),this.querySelector("#collapse-btn").addEventListener("click",()=>this.toggleCollapse()),this.querySelector("#expand-btn").addEventListener("click",()=>this.toggleCollapse())}_wireDialogs(){let e=this.querySelector("#new-file-dlg"),s=this.querySelector("#new-file-title"),t=this.querySelector("#new-file-error"),a=this.querySelector("#new-file-hint"),n=p=>{p?(t.textContent=p,t.classList.remove("hidden")):(t.classList.add("hidden"),t.textContent="")};e.querySelector('[data-action="close"]').addEventListener("click",()=>e.close()),e.querySelector('[data-action="cancel"]').addEventListener("click",()=>e.close()),e.querySelector('[data-action="save"]').addEventListener("click",async()=>{let p=s.value.trim()||"\u672A\u547D\u540D\u6587\u4EF6";try{let w=Date.now(),S={id:P("fl"),folderId:this._pendingNewFileFolderId??null,title:p,text:"",segments:[],autoAll:!1,createdAt:w,updatedAt:w};await B.put(S),s.value="",n(""),S.folderId&&this._expanded.add(S.folderId),e.close(),await this.refresh(),this.setSelected(S.id,{silent:!0}),this._fireSelect(),this._toast(`\u5DF2\u65B0\u5EFA\u300C${p}\u300D`,"ok")}catch(w){n("\u521B\u5EFA\u5931\u8D25:"+(w?.message||w))}}),s.addEventListener("input",()=>n("")),s.addEventListener("keydown",p=>{p.key==="Enter"&&(p.preventDefault(),e.querySelector('[data-action="save"]').click())}),this._newFileDlg=e,this._newFileHint=a;let i=this.querySelector("#new-folder-dlg"),x=this.querySelector("#new-folder-name"),d=this.querySelector("#new-folder-error"),l=this.querySelector("#new-folder-hint"),r=p=>{p?(d.textContent=p,d.classList.remove("hidden")):(d.classList.add("hidden"),d.textContent="")};i.querySelector('[data-action="close"]').addEventListener("click",()=>i.close()),i.querySelector('[data-action="cancel"]').addEventListener("click",()=>i.close()),i.querySelector('[data-action="save"]').addEventListener("click",async()=>{let p=x.value.trim();if(!p){r("\u540D\u5B57\u4E0D\u80FD\u4E3A\u7A7A");return}try{await R.put({id:P("f"),parentId:this._pendingNewFolderParentId??null,name:p,createdAt:Date.now(),updatedAt:Date.now()}),x.value="",r(""),this._pendingNewFolderParentId&&this._expanded.add(this._pendingNewFolderParentId),i.close(),await this.refresh(),this._toast(`\u5DF2\u521B\u5EFA\u300C${p}\u300D`,"ok")}catch(w){r("\u521B\u5EFA\u5931\u8D25:"+(w?.message||w))}}),x.addEventListener("input",()=>r("")),x.addEventListener("keydown",p=>{p.key==="Enter"&&(p.preventDefault(),i.querySelector('[data-action="save"]').click())}),this._newFolderDlg=i,this._newFolderHint=l,this._renameTarget=null;let o=this.querySelector("#rename-dlg"),f=this.querySelector("#rename-input"),m=this.querySelector("#rename-error"),g=this.querySelector("#rename-title"),M=p=>{p?(m.textContent=p,m.classList.remove("hidden")):(m.classList.add("hidden"),m.textContent="")};o.querySelector('[data-action="close"]').addEventListener("click",()=>o.close()),o.querySelector('[data-action="cancel"]').addEventListener("click",()=>o.close()),o.querySelector('[data-action="save"]').addEventListener("click",async()=>{let p=f.value.trim();if(!p){M("\u540D\u5B57\u4E0D\u80FD\u4E3A\u7A7A");return}if(!this._renameTarget){o.close();return}let{kind:w,id:S}=this._renameTarget;try{if(w==="file"){let h=await B.get(S);if(!h)throw new Error("\u6587\u4EF6\u4E0D\u5B58\u5728");h.title=p,h.updatedAt=Date.now(),await B.put(h)}else{let h=await R.get(S);if(!h)throw new Error("\u6587\u4EF6\u5939\u4E0D\u5B58\u5728");h.name=p,h.updatedAt=Date.now(),await R.put(h)}M(""),o.close(),await this.refresh(),this._toast("\u5DF2\u91CD\u547D\u540D","ok")}catch(h){M("\u91CD\u547D\u540D\u5931\u8D25:"+(h?.message||h))}}),f.addEventListener("input",()=>M("")),f.addEventListener("keydown",p=>{p.key==="Enter"&&(p.preventDefault(),o.querySelector('[data-action="save"]').click())}),this._renameDlg=o,this._renameInput=f,this._renameTitle=g,this._moveTarget=null,this._moveSelectedFolderId=null;let E=this.querySelector("#move-dlg"),q=this.querySelector("#move-tree");E.querySelector('[data-action="close"]').addEventListener("click",()=>E.close()),E.querySelector('[data-action="cancel"]').addEventListener("click",()=>E.close()),E.querySelector('[data-action="save"]').addEventListener("click",async()=>{if(!this._moveTarget){E.close();return}let{kind:p,id:w}=this._moveTarget,S=this._moveSelectedFolderId??null;try{p==="file"?await ne(w,S):await de(w,S),E.close(),await this.refresh(),this._toast("\u5DF2\u79FB\u52A8","ok")}catch(h){this._toast("\u79FB\u52A8\u5931\u8D25:"+(h?.message||h),"err")}}),this._moveDlg=E,this._moveTree=q}_openNewFile(e=null){if(this._pendingNewFileFolderId=e,this.querySelector("#new-file-title").value="",this.querySelector("#new-file-error").classList.add("hidden"),e){let s=this._allFolders.find(a=>a.id===e),t=this._folderPathLabel(e);this._newFileHint.textContent=`\u5C06\u4FDD\u5B58\u5230:${t}${s?` (${s.name})`:""}`,this._newFileHint.classList.remove("hidden")}else this._newFileHint.classList.add("hidden");this._newFileDlg.showModal(),setTimeout(()=>this.querySelector("#new-file-title").focus(),0)}_openNewFolder(e=null){if(this._pendingNewFolderParentId=e,this.querySelector("#new-folder-name").value="",this.querySelector("#new-folder-error").classList.add("hidden"),e){let s=this._folderPathLabel(e);this._newFolderHint.textContent=`\u5C06\u521B\u5EFA\u5230:${s}`,this._newFolderHint.classList.remove("hidden")}else this._newFolderHint.classList.add("hidden");this._newFolderDlg.showModal(),setTimeout(()=>this.querySelector("#new-folder-name").focus(),0)}_folderPathLabel(e){let s=[],t=e,a=new Set;for(;t!=null&&!a.has(t);){a.add(t);let n=this._allFolders.find(i=>i.id===t);if(!n)break;s.unshift(n.name),t=n.parentId??null}return["\u6839\u76EE\u5F55",...s].join(" / ")}_openRename(e,s){this._renameTarget={kind:e,id:s};let t=e==="file"?this._allFiles.find(a=>a.id===s)?.title:this._allFolders.find(a=>a.id===s)?.name;this._renameTitle.textContent=e==="file"?"\u91CD\u547D\u540D\u6587\u4EF6":"\u91CD\u547D\u540D\u6587\u4EF6\u5939",this._renameInput.value=t||"",this.querySelector("#rename-error").classList.add("hidden"),this._renameDlg.showModal(),setTimeout(()=>{this._renameInput.focus(),this._renameInput.select()},0)}_openMove(e,s){this._moveTarget={kind:e,id:s};let t=e==="file"?this._allFiles.find(a=>a.id===s)?.folderId??null:this._allFolders.find(a=>a.id===s)?.parentId??null;this._moveSelectedFolderId=t,this._renderMoveTree(),this._moveDlg.showModal()}_renderMoveTree(){let e=this._moveTree,s=this._sortRows(this._allFolders,i=>i.name),t=(i,x)=>{let d=s.filter(r=>r.parentId===i.id);return`
        <div class="flex items-center gap-1 px-1 py-0.5 rounded cursor-pointer ${this._moveSelectedFolderId===i.id?"bg-blue-100 dark:bg-blue-900/40":"hover:bg-white dark:hover:bg-slate-800"}"
             data-move-folder-id="${_(i.id)}" data-depth="${x}">
          <span class="shrink-0 w-4"></span>
          <span class="shrink-0 text-slate-500">${b.folder}</span>
          <span class="flex-1 truncate">${_(i.name)}</span>
        </div>
        ${d.map(r=>t(r,x+1)).join("")}
      ...
      ${a.map(i=>t(i,1)).join("")}
    `,e.querySelectorAll("[data-move-folder-id]").forEach(i=>{i.addEventListener("click",()=>{this._moveSelectedFolderId=i.dataset.moveFolderId||null,this._renderMoveTree()})})}async _deleteFile(e){let s=this._allFiles.find(t=>t.id===e);if(s&&confirm(`\u786E\u8BA4\u5220\u9664\u6587\u4EF6\u300C${s.title}\u300D?`))try{await B.del(e),this._selectedFileId===e&&(this._selectedFileId=null,this._fireSelect()),this._toast(`\u5DF2\u5220\u9664\u300C${s.title}\u300D`,"ok"),await this.refresh()}catch(t){this._toast("\u5220\u9664\u5931\u8D25:"+(t?.message||t),"err")}}async _deleteFolder(e){let s=this._allFolders.find(d=>d.id===e);if(!s)return;let t=this._allFolders,a=new Set([e]),n=!0;for(;n;){n=!1;for(let d of t)d.parentId!=null&&a.has(d.parentId)&&!a.has(d.id)&&(a.add(d.id),n=!0)}let i=this._allFiles.filter(d=>d.folderId!=null&&a.has(d.folderId)).length;if(confirm(`\u786E\u8BA4\u5220\u9664\u6587\u4EF6\u5939\u300C${s.name}\u300D?
\u5C06\u540C\u65F6\u5220\u9664 ${a.size-1} \u4E2A\u5B50\u6587\u4EF6\u5939 + ${i} \u4E2A\u6587\u4EF6,\u4E14\u4E0D\u53EF\u64A4\u9500\u3002`))try{let d=await oe(e);this._selectedFileId&&d.files.includes(this._selectedFileId)&&(this._selectedFileId=null,this._fireSelect()),this._toast(`\u5DF2\u5220\u9664 (${d.folders.length} \u76EE\u5F55, ${d.files.length} \u6587\u4EF6)`,"ok"),await this.refresh()}catch(d){this._toast("\u5220\u9664\u5931\u8D25:"+(d?.message||d),"err")}}async _duplicateFile(e){let s=this._allFiles.find(n=>n.id===e);if(!s)return;let t=Date.now(),a={...s,id:P("fl"),title:`${s.title} (\u526F\u672C)`,createdAt:t,updatedAt:t};try{await B.put(a),this._toast("\u5DF2\u590D\u5236","ok"),await this.refresh()}catch(n){this._toast("\u590D\u5236\u5931\u8D25:"+(n?.message||n),"err")}}_wireCtxMenu(){this._ctxMenu=this.querySelector("#ctx-menu"),this._onDocClickHideMenu=()=>this._hideCtxMenu(),this._onEscHideMenu=e=>{e.key==="Escape"&&this._hideCtxMenu()},document.addEventListener("click",this._onDocClickHideMenu),document.addEventListener("keydown",this._onEscHideMenu)}_showFolderMenu(e,s,t){this._showCtxMenu(e,s,[{label:"\u65B0\u5EFA\u6587\u4EF6",icon:b.filePlus,onClick:()=>this._openNewFile(t)},{label:"\u65B0\u5EFA\u6587\u4EF6\u5939",icon:b.folderPlus,onClick:()=>this._openNewFolder(t)},{label:"\u91CD\u547D\u540D",icon:b.edit,onClick:()=>this._openRename("folder",t)},{label:"\u79FB\u52A8\u5230...",icon:b.move,onClick:()=>this._openMove("folder",t)},{label:"\u5220\u9664",icon:b.trash,danger:!0,onClick:()=>this._deleteFolder(t)}])}_showCtxMenu(e,s,t){let a=this._ctxMenu;a.innerHTML=t.map((o,f)=>`
      <button type="button" data-ctx-i="${f}"
              class="w-full text-left px-3 py-1.5 flex items-center gap-2 hover:bg-slate-100 dark:hover:bg-slate-700 ${o.danger?"text-red-600 dark:text-red-400":""}">
        <span class="shrink-0">${o.icon||""}</span><span>${_(o.label)}</span>
      </button>
    `).join(""),a.classList.remove("hidden");let n=180,i=t.length*32+8,x=window.innerWidth,d=window.innerHeight,l=Math.min(e,x-n-4),r=Math.min(s,d-i-4);a.style.left=`${l}px`,a.style.top=`${r}px`,a.querySelectorAll("[data-ctx-i]").forEach(o=>{o.addEventListener("click",f=>{f.stopPropagation();let m=Number(o.dataset.ctxI);a.classList.add("hidden"),t[m].onClick()})})}_hideCtxMenu(){this._ctxMenu?.classList.add("hidden")}_toast(e,s="info"){if(this._onToast)try{this._onToast(e,s);return}catch(a){console.error("[file-sidebar] onToast threw:",a)}let t=document.getElementById("__filesb-toast");t||(t=document.createElement("div"),t.id="__filesb-toast",t.className="fixed top-14 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-md shadow-lg text-sm transition-opacity duration-300",document.body.appendChild(t)),t.textContent=e,t.className="fixed top-14 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-md shadow-lg text-sm transition-opacity duration-300 "+(s==="ok"?"bg-green-600 text-white":s==="err"?"bg-red-600 text-white":"bg-slate-800 text-white"),t.style.opacity="1",clearTimeout(this._toastTimer),this._toastTimer=setTimeout(()=>{t.style.opacity="0",setTimeout(()=>{try{t.remove()}catch{}},300)},1800)}};function _(L){return String(L??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}customElements.define("file-sidebar",Y);N.addRoute("/",()=>import("./speak.js"));N.addRoute("/speak",()=>import("./speak.js"));N.addRoute("/404",()=>import("./speak.js"));console.info("[app] starting, port=",F("PORT","?"),"API_BASE=",F("API_BASE","(none)"));N.start();window.__app={router:N};
