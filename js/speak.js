...
\u98CE\u683C\u8981\u6C42:${{"\u77ED\u6587 \xB7 \u77E5\u8BC6\u79D1\u666E":"\u901A\u4FD7\u8BB2\u89E3\u578B,\u76EE\u6807\u7EA6 200 \u5B57,3~5 \u6BB5,\u9002\u5408\u5927\u4F17\u542C\u4F17\u3002","\u4E2D\u7BC7 \xB7 \u6545\u4E8B\u53D9\u8FF0":"\u53D9\u4E8B\u7ED3\u6784,\u6709\u8D77\u627F\u8F6C\u5408,\u76EE\u6807\u7EA6 500 \u5B57,5~8 \u6BB5\u3002","\u957F\u6587 \xB7 \u6DF1\u5EA6\u8BB2\u89E3":"\u5C42\u5C42\u9012\u8FDB\u3001\u7531\u6D45\u5165\u6DF1,\u76EE\u6807\u7EA6 1000 \u5B57,8~12 \u6BB5\u3002","\u77ED\u8BD7 \xB7 \u53E4\u98CE\u4EFF\u4F5C":"\u53E4\u8BD7 / \u8BCD\u98CE\u683C,4~8 \u53E5,\u6BCF\u53E5 5~7 \u5B57\u6216 7 \u5B57\u4E3A\u4E3B,\u62BC\u97F5\u3002","\u6F14\u8BB2 \xB7 \u603B\u7ED3\u9648\u8BCD":"\u6177\u6168\u6FC0\u6602\u3001\u6709\u53F7\u53EC\u529B,\u76EE\u6807\u7EA6 300 \u5B57,4~6 \u6BB5\u3002"}[t]||"\u76EE\u6807\u7EA6 400 \u5B57,5~7 \u6BB5\u3002"}
...
\u3010\u98CE\u683C\u3011${t}`}],temperature:.7,stream:!1}),signal:h.signal})}finally{clearTimeout(x)}if(!o.ok){let _=await o.text().catch(()=>o.statusText);throw new Error(`HTTP ${o.status} \u2014 ${_.slice(0,200)}`)}let n=(await o.json().catch(()=>null))?.choices?.[0]?.message?.content;if(typeof n!="string"||!n.trim())throw new Error("AI \u8FD4\u56DE\u4E3A\u7A7A");let y=null,u=n.match(/```(?:json)?\s*([\s\S]*?)```/i);for(let _ of[u?.[1],n])if(_)try{let T=JSON.parse(_.trim());if(T&&Array.isArray(T.segments)){y=T;break}}catch{}if(!y)throw new Error("AI \u8FD4\u56DE\u65E0\u6CD5\u89E3\u6790\u4E3A {title, segments[]} JSON");let f=String(y.title||"").trim().replace(/^[\s"'""''「」『』《》【】\[\]]+/,"").replace(/[\s"'""''「」『』《》【】\[\]]+$/,"").replace(/\s+/g," ").slice(0,64)||this._currentFile.title||L(e,32),v=(y.segments||[]).map(_=>String(_||"").replace(/\s+/g," ").trim()).filter(Boolean).map(_=>_.length>500?_.slice(0,500):_);if(v.length===0)throw new Error("AI \u672A\u8FD4\u56DE\u4EFB\u4F55\u6BB5\u843D,\u8BF7\u91CD\u8BD5");let C=v.join(`

`);await this._applyAiGenerated({title:f,text:C,segments:v}),this._aiGenDlg.close(),this._toast(`\u5DF2\u91CD\u65B0\u751F\u6210\u300C${f}\u300D(${v.length} \u6BB5)`,"ok")}catch(i){let c=i?.message||String(i);i?.name==="AbortError"?this._setAiGenErr("AI \u751F\u6210\u8D85\u65F6 (90s),\u8BF7\u68C0\u67E5\u7F51\u7EDC\u6216 AI \u670D\u52A1"):i instanceof TypeError?this._setAiGenErr("\u7F51\u7EDC\u9519\u8BEF:"+c+" (\u53EF\u80FD\u662F CORS)"):this._setAiGenErr("AI \u751F\u6210\u5931\u8D25:"+c),console.error("[speak] AI generate failed:",i)}finally{r.disabled=!1,r.innerHTML=l,this._aiGenTopic.disabled=!1,this._aiGenStyle.disabled=!1,this._setAiGenStatus("")}},async _copyAiPrompt(){if(!this._currentFile)return;let e=(this._aiGenMTopic.value||this._aiGenTopic.value).trim(),t=this._aiGenMStyle.value||this._aiGenStyle.value;if(!e){this._setAiGenMErr("\u8BF7\u5148\u586B\u5199\u4E3B\u9898"),this._aiGenMTopic.focus();return}let s=this._buildManualPromptText(e,t),a=this._aiGenMCopyBtn,r=a.textContent,l=!1;try{navigator.clipboard?.writeText&&(await navigator.clipboard.writeText(s),l=!0)}catch(i){console.warn("[speak] clipboard.writeText failed:",i)}if(!l)try{let i=document.createElement("textarea");i.value=s,i.style.position="fixed",i.style.top="-9999px",document.body.appendChild(i),i.focus(),i.select(),l=document.execCommand("copy"),document.body.removeChild(i)}catch(i){console.warn("[speak] execCommand copy fallback failed:",i)}l?(a.textContent="\u2713 \u5DF2\u590D\u5236",this._setAiGenMErr(""),this._setAiGenMStatus("\u63D0\u793A\u8BCD\u5DF2\u590D\u5236\u5230\u526A\u8D34\u677F,\u5230 ChatGPT/Claude \u7B49\u7C98\u8D34\u5373\u53EF"),clearTimeout(this._copyResetTimer),this._copyResetTimer=setTimeout(()=>{a.textContent=r},2e3)):(this._setAiGenMErr("\u590D\u5236\u5931\u8D25,\u8BF7\u624B\u52A8\u9009\u4E2D\u4E0B\u65B9\u63D0\u793A\u8BCD\u6587\u672C\u590D\u5236"),this._aiGenMResponse.value=s+`

(\u4EE5\u4E0A\u662F\u63D0\u793A\u8BCD\u6A21\u677F,AI \u56DE\u590D\u8BF7\u7C98\u8D34\u5230\u4E0B\u65B9 \u2193)
`)},async _parseAiResponse(){if(!this._currentFile)return;let e=(this._aiGenMResponse.value||"").trim();if(!e){this._setAiGenMErr("\u8BF7\u5148\u7C98\u8D34 AI \u7684\u56DE\u590D"),this._aiGenMResponse.focus();return}let t=e.replace(/^﻿/,"").replace(/[-‍﻿]/g,"");t=t.replace(/<think>[\s\S]*?<\/think>/gi,"").trim();let a=[t.match(/```(?:json)?\s*([\s\S]*?)```/i)?.[1],t],r=t.indexOf("{"),l=t.lastIndexOf("}");r>=0&&l>r&&a.push(t.slice(r,l+1));let i=t.indexOf("["),c=t.lastIndexOf("]");i>=0&&c>i&&a.push(t.slice(i,c+1));let h=null;for(let u of a)if(u)try{let b=JSON.parse(u.trim());if(b&&Array.isArray(b.segments)){h=b;break}}catch{}if(!h){for(let u of a)if(u)try{let b=JSON.parse(u.trim());if(Array.isArray(b)){h={title:"",segments:b};break}}catch{}}if(!h){this._setAiGenMErr("\u65E0\u6CD5\u89E3\u6790\u4E3A {title, segments[]} JSON,\u8BF7\u786E\u8BA4\u590D\u5236\u5B8C\u6574(\u652F\u6301 json \u4EE3\u7801\u5757)");return}let x=(this._aiGenMTopic.value||this._aiGenTopic.value).trim(),p=String(h.title||"").trim().replace(/^[\s"'""''「」『』《》【】\[\]]+/,"").replace(/[\s"'""''「」『』《》【】\[\]]+$/,"").replace(/\s+/g," ").slice(0,64)||this._currentFile.title||L(x||h.segments?.[0]||"AI \u751F\u6210\u6717\u8BFB\u7A3F",32),n=(h.segments||[]).map(u=>String(u||"").replace(/\s+/g," ").trim()).filter(Boolean).map(u=>u.length>500?u.slice(0,500):u);if(n.length===0){this._setAiGenMErr("\u89E3\u6790\u6210\u529F\u4F46\u6BB5\u843D\u4E3A\u7A7A,\u8BF7\u91CD\u8BD5");return}let y=n.join(`

`);this._setAiGenMStatus("\u6B63\u5728\u4FDD\u5B58..."),await this._applyAiGenerated({title:p,text:y,segments:n}),this._aiGenDlg.close(),this._toast(`\u5DF2\u91CD\u65B0\u751F\u6210\u300C${p}\u300D(${n.length} \u6BB5)`,"ok")},async _applyAiGenerated({title:e,text:t,segments:s}){let a=Date.now(),r={...this._currentFile,title:e,text:t,segments:s,updatedAt:a};await P.put(r),this._currentFile=r,this._currentFileId=r.id,g.set(G,r.id),this._text=t,this._segments=s,this._saveState(),this._stopAll({silent:!0}),this._updateMeta(),this._renderLyrics();let l=this._root.querySelector("#file-sidebar");l&&await l.refresh(),this._refreshAiGenBtnState()},_toast(e,t="info"){let s=document.getElementById("__speak-toast");s||(s=document.createElement("div"),s.id="__speak-toast",s.style.transition="opacity 300ms",document.body.appendChild(s)),s.textContent=e,s.className="fixed top-12 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-md shadow-lg text-sm "+(t==="ok"?"bg-green-600 text-white":t==="err"?"bg-red-600 text-white":"bg-slate-800 text-white"),s.style.opacity="1",clearTimeout(this._toastTimer),this._toastTimer=setTimeout(()=>{s.style.opacity="0",setTimeout(()=>{try{s.remove()}catch{}},320)},1800)}};function H(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function N(e){let t=String(e||"");return/[一-鿿぀-ゟ゠-ヿ가-힯]/.test(t)?"zh":"en"}function R(e,t){let a=[e.match(/```(?:json)?\s*([\s\S]*?)```/i)?.[1],e];for(let l of a)if(l)try{let i=JSON.parse(l.trim());if(Array.isArray(i))return i.filter(c=>typeof c=="string"&&c.trim());if(i&&Array.isArray(i.segments))return i.segments.filter(c=>typeof c=="string"&&c.trim())}catch{}let r=e.split(/\r?\n/).map(l=>l.trim()).filter(l=>l&&!l.startsWith("{")&&!l.startsWith("["));return r.length>=2?r:$.split(t)}var W=D;export{W as default,D as page};
